<?php
	...
	class PDF extends FPDF{
		...
		function Header(){
			...
			$this->Cell(190,7,'REPORT TYPE: Expulsions '.$reportype,0,0,'C');
			$this->Ln(20);
		}
	...
	$query = "select * from goalstable where serialno>0 and activity='Expulsion' ";
	if($reportype=="List by No of Expulsions") $query="select count(*) as expulsions, serialno, regnumber, playername, clubname, season, leaguename, goaltime, weeks, matchno from goalstable where serialno>0 and activity='Expulsion' ";
	if($reportype=="List by Clubs") $query="select count(*) as expulsions, serialno, regnumber, playername, clubname, season, leaguename, goaltime, weeks, matchno from goalstable where serialno>0 and activity='Expulsion' ";
	...
	if($reportype=="List by No of Expulsions") $query .= " group by regnumber order by expulsions desc, clubname, playername ";
	if($reportype=="List by Time of Expulsions") $query .= " order by goaltime ";
	if($reportype=="List by Weeks") $query .= " order by weeks, clubname ";
	if($reportype=="List by Clubs") $query .= " group by clubname order by expulsions desc, clubname ";
	...
	if(mysql_num_rows($result) > 0){
		...
		while ($row = mysql_fetch_array($result)) {
			...
			if($printheader=="Yes"){
				...
				if($reportype=="List by Time of Expulsions" || $reportype=="List by Weeks" || $reportype=="List by No of Expulsions"){
					$pdf->Cell(40,5,"LICENSE NO",1,0,'L');
					$pdf->Cell(60,5,"PLAYER",1,0,'L');
				}
				$pdf->Cell(30,5,"CLUB",1,0,'L');
				if($reportype=="List by Time of Expulsions" || $reportype=="List by Weeks"){
					$pdf->Cell(15,5,"WEEK",1,0,'C');
					$pdf->Cell(25,5,"MATCH NO",1,0,'C');
					$pdf->Cell(15,5,"TIME",1,0,'C');
				}
				if($reportype=="List by No of Expulsions" || $reportype=="List by Clubs"){
					$pdf->Cell(40,5,"NO OF EXPULSIONS",1,0,'C');
				}
				$pdf->Ln(5);
				$count=1;
				$pagelinecounter++;
			...
			if($reportype=="List by No of Expulsions" || $reportype=="List by Clubs"){
				$pdf->Cell(40,5,$expulsions,1,0,'C');
			}
			$pdf->Ln(5);
			$pagelinecounter++;
		}
