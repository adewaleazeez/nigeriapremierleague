<?php
	...
	class PDF extends FPDF{
		...
		function Header(){
			...
			$this->Cell(190,7,'THE MATCHES RESULTS',0,0,'C');
			$this->Ln();
			//if($leaguenames!=null && $leaguenames!=""){
	...
	$query .= "from matchestable where serialno>0 ";
	if($seasons != "") $query .= " and season='{$seasons}' "; 
	if($leaguenames != "") $query .= " and leaguename='{$leaguenames}' "; 
	...
	if(mysql_num_rows($result) > 0){
		...
		while ($row = mysql_fetch_array($result)) {
			...
			if($printheader=="Yes"){
				...
				$pdf->Cell(50,5,"HOST TEAM",1,0,'R');
				$pdf->Cell(50,5,"VISITING TEAM",1,0,'L');
				$pdf->Cell(14,5,"WEEK",1,0,'C');
				$pdf->Cell(22,5,"MATCH NO",1,0,'C');
				$pdf->Cell(26,5,"MATCH DATE",1,0,'C');
				$pdf->Cell(22,5,"KICK OFF",1,0,'C');
				$pdf->Ln(5);
				$count=1;
				$pagelinecounter++;
			}
			...
			$visitorscores=$visitorscore;
			if($playflag!="Yes"){
				$hostscores=" ";
				$visitorscores=" ";
			}
			$pdf->Cell(50,5,$hostname."  [ ".$hostscores." ]",1,0,'R');
			$pdf->Cell(50,5,"[ ".$visitorscores." ]  ".$visitorname,1,0,'L');
			$pdf->Cell(14,5,$weeks,1,0,'C');
			$pdf->Cell(22,5,$matchno,1,0,'C');
			$matchdates=substr($matchdate,8,2)."/".substr($matchdate,5,2)."/".substr($matchdate,0,4);
			$kickoff=substr($matchdate,11,8);
			$pdf->Cell(26,5,$matchdates,1,0,'C');
			$pdf->Cell(22,5,$kickoff,1,0,'C');
			$pdf->Ln(5);
			$pagelinecounter++;
		}
	}
